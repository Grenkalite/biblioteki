...
col_pressed = 0xFF8DC100 	-- цвет нажатия
...
function main()
	...
	sampAddChatMessage("[KeyLogger]{FFFFFF} Автор: {8DC100}Cosmo{FFFFFF} | Активация: {8DC100}/kl", 0x8DC100)
	sampRegisterChatCommand("kl", function() 
		config.m.kl = not config.m.kl; ini.save(config, 'keylogger.ini')
		sampAddChatMessage("KeyLogger: {FFFFFF}" .. (config.m.kl and "Включен" or "Выключен"), col_pressed)
	end)
	sampRegisterChatCommand("kt", function() 
		config.m.kt = not config.m.kt; ini.save(config, 'keylogger.ini')
		sampAddChatMessage("KeyTimer: {FFFFFF}" .. (config.m.kt and "Включен" or "Выключен"), col_pressed)
	end)
	wait(-1)
end
