...
function main()
    ...
    while true do        
        ...
        imgui.Process = to.v or settings.v
        wait(0)
    end
end
...
function imgui.OnDrawFrame()
    ...
    if settings.v then
        ...
        imgui.Begin(u8'Timer Online by Cosmo', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.ShowBorders)
...
function time()
    ...
    while true do
        ...
        if sampGetGamestate() == 3 then 								-- Игровой статус равен "Подключён к серверу" (Что бы онлайн считало только, когда, мы подключены к серверу)
	        nowTime = os.date("%H:%M:%S", os.time()) 					-- Текущее время
	    	...
	    	startTime = startTime + 1									-- Прибвляем к началу таймера время которое мы подключаемся
	    end
    end
end
